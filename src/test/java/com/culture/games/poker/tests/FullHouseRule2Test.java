...
public class FullHouseRule2Test {
	
	@Test
	public void testFullHouseRule(){
		
		LinkedList<PlayingCard> testBoard = new LinkedList<>();
		testBoard.add(new PlayingCard(Suit.SPADES,Rank.EIGHT));
		testBoard.add(new PlayingCard(Suit.DIAMONDS,Rank.KING));
		testBoard.add(new PlayingCard(Suit.HEARTS,Rank.EIGHT));
		...
		player.addCardToHand(new PlayingCard(Suit.CLUBS,Rank.KING));
		...
		cardHandRules.getResult().ifPresent(foo -> {
			if(foo.getValue() instanceof HandType){
				...
				if(playerHand.equals(HandType.FULL_HOUSE)){
